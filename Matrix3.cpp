...
Matrix3 rotMatrixVectors(Vec3 a, Vec3 b) {
	...
	rotMatrix.matrix[0][1] = axis.x*axis.y*(1.0 - c) - axis.z*s;
	rotMatrix.matrix[0][2] = axis.x*axis.z*(1.0 - c) + axis.y*s;
	rotMatrix.matrix[1][0] = rotMatrix.matrix[0][1] + 2 * axis.z*s;
	rotMatrix.matrix[1][1] = c + axis.y*axis.y*(1.0 - c);
	rotMatrix.matrix[1][2] = axis.y*axis.z*(1.0 - c) - axis.x*s;
	rotMatrix.matrix[2][0] = rotMatrix.matrix[0][2] - 2 * axis.y*s;
	rotMatrix.matrix[2][1] = rotMatrix.matrix[1][2] + 2 * axis.x*s;
	rotMatrix.matrix[2][2] = c + axis.z*axis.z*(1.0 - c);
	return rotMatrix;
}
